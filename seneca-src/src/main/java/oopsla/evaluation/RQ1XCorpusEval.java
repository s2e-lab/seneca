...
import oopsla.evaluation.utils.EvalMonitor;
import oopsla.evaluation.utils.XCorpusTestCases;
...
import static oopsla.evaluation.utils.EvaluationUtil.*;
...
public class RQ1XCorpusEval {
    ...
    public static List<CallGraph> computeCallGraphs(String projectPath, Approach approach, String approachName, PointerAnalysisPolicy policy) throws ClassHierarchyException, IOException {
    ...
    private static boolean isTestCaseEntryPoint(Entrypoint ep) {
        return ep.getMethod().getDeclaringClass().getName().toString().startsWith("Ltestcases/TC_");
    }


    public void runXCorpusEval() throws IOException, ClassHierarchyException {
        printHeader();
        for (String projectPath : XCORPUS_TCS) {
            for (Approach algorithm : Approach.values()) {
                for (PointerAnalysisPolicy policy : policies) {
                    ...
                    String testName = join("\t", projectName, approachName, policy.toString());
