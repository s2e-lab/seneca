...
import edu.rit.se.design.callgraph.analysis.seneca.*;
 ...
 * This class only propagates taint to application and extension scope.
 *
 * For Primordial, it simply abstracts away common collection classes and
 *
 * @author Joanna C. S. Santos
 */
public abstract class BoundedSenecaCallGraphBuilder extends AbstractSerializationCallGraphBuilder implements ITaintedCallGraphBuilder {
     ...
     */
    protected BoundedSenecaCallGraphBuilder(
    ...
    protected IPointsToSolver makeSolver() {
        TaintedSerializationHandler serializationHandler = new TaintedSerializationHandler(this);
     ...
     *
     * @param appContextInterpreter The context interpreter to use in the primary
     * @return The SSAContextInterpreter for the specific class
     */
    protected abstract SSAContextInterpreter makeDefaultContextInterpreter(SSAContextInterpreter appContextInterpreter);
    //</editor-fold>
    ...
    protected boolean isNodeInRightScope(CGNode n) {
        return isApplicationScope(n) || isExtensionScope(n);
    }
    @Override
...
//        if(WalaUtils.isPrimordialScope(node))
//            return super.makeVisitor(node);
        return new BoundedTaintedVisitor(this, node);
    }

    public class BoundedTaintedVisitor extends ConstraintVisitor {

        public BoundedTaintedVisitor(SSAPropagationCallGraphBuilder builder, CGNode node) {
            super(builder, node);
        }
        ...
        @Override
        // This can cause taints, but call graphs can be constructed without Pi
